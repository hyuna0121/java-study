...
public class HashSetExample {
	public static void main(String[] args) {
		...
		// JVM의 내부 해시 구현에 의해 우연히 정렬된 것처럼 출력된 것 일 뿐 순서를 보장하지 않음
...
//		Set<String> set = new HashSet<String>();
		...
		while (iterator.hasNext()) { // 가져올 객체가 있으면 true, 없으면 false
			...
			if ("Java".equals(element)) iterator.remove(); // 참고 : remove()는 next() 호출 이후에만 호출 가능하며, 한 번만 가능
		}
		System.out.println(set);
		...
		// 모든 객체를 제거하고 비움
		set.clear();
		if (set.isEmpty()) { // 컬렉션이 비어 있는 지 조사
		...
		class2.add(2);
		class2.add(3);
		class2.add(4); // 컬렉션은 객체만 저장하므로 자동 박싱, 언박싱됨
