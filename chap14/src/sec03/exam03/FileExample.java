...
//		if (!dirTest.exists()) dirTest.mkdir(); // 경로상 맨뒤 폴더만 생성(앞에 상위 폴더가 있어야 함)
		...
		if (dirTest.delete()) System.out.println("videos 폴더 삭제 완료");
		dirTest = new File("C:/Temp/test");
		if (dirTest.delete()) System.out.println("test 폴더 삭제 완료");
		...
		if (temp.exists() && temp.isDirectory()) {
			...
			for (File file : contents) {
				...
				// 날짜 형식으로 바꾸기 위해 Date 클래스와 SimpleDataFormat 활용
