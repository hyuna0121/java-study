...
public class UUIDExample {
	public static void main(String[] args) {
		...
		// 사용자가 파일을 업로드 할 때, 동일한 이름의 파일이 존재하면 덮어쓰기 위험이 있으므로
		// UUID를 활용해 고유한 이름으로 저장
		// DB에 원본 파일명은 따로 기록해두고, 파일 서버에는 랜덤한 파일명으로 올림
		String uploadDirPath = "D:/upload";
		...
		File dest = new File(uploadDirPath, savedName);
		
		// 가상의 저장 시뮬레이션
		try {
			boolean created = dest.createNewFile();
			
			if (created) System.out.println("저장 파일 명 : " + dest.getName());
			else System.out.println("파일 생성 실패(이미 존재)");
		}catch (Exception e) {
			e.printStackTrace();
		}
	}
